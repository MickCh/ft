...
- `src/main.rs` — two output paths: `run_streaming` (file or stdout) and `run_in_place` (`--in-place`, writes to a `.<name>.ft-<pid>.tmp` sibling — opened with `create_new` so a planted file or symlink at that name is refused — then atomically `rename`s it over the input, cleaning up the temp file on any error). `--in-place` requires a file input and conflicts with `--output`.
